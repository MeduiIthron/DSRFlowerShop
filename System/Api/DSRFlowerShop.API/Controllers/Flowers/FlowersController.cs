...
public class FlowersController : ControllerBase
{
    ...
    public async Task<IEnumerable<FlowerResponse>> GetFlowers([FromRoute] int DealerID, [FromQuery] int offset = 0, [FromQuery] int limit = 10)
    {
    ...
    public async Task<FlowerResponse> AddFlower(AddFlowerRequest request)
    {
        var model = mapper.Map<AddFlowerModel>(request);
    ...
    public async Task<IActionResult> UpdateFlower([FromRoute] int id, [FromBody] UpdateFlowerRequest request)
    {
        ...
        await FlowerService.UpdateFlower(request.DealerID, id, model);
